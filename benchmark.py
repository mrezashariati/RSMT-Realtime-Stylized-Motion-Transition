...
class BatchRotateYCenterXZ(torch.nn.Module):
    ...
    def forward(self, global_positions, global_quats, ref_frame_id):
        ...
        ref_vector = torch.cross(v1, v2.expand_as(v1), dim=-1)
...
class GetLastRotLoss(GetLoss):
    ...
    def __call__(self, data, mA, mB):
        ...
        b = data[mB]["rot"]
        return torch.mean(
            torch.sqrt(torch.sum((a[:, -1:, ...] - b[:, -1:, ...]) ** 2.0, dim=(2, 3)))
        )
...
class GetLastPosLoss(GetLoss):
    ...
    def __call__(self, data, mA, mB):
        ...
        return torch.mean(
            torch.sqrt(torch.sum((a[:, -1:, ...] - b[:, -1:, ...]) ** 2.0, dim=(2)))
        )
...
class GetPosLoss(GetLoss):
    ...
    def __call__(self, data, mA, mB):
        ...
        b = (b - self.x_mean) / self.x_std
        return torch.mean(
            torch.sqrt(torch.sum((a[:, :, ...] - b[:, :, ...]) ** 2.0, dim=(2)))
        )
    ...
    def __call__(self, data, mA, mB):
        ...
        for i, length_noaug in enumerate(self.style_seq):
            if length_noaug:
                ...
                latentA = (
                    data[mA]["latent"][start_id_noaug : start_id_noaug + length_noaug]
                    / self.norm_factor
                )
                ...
                latentB = (
                    data[mB]["latent"][start_id_aug : start_id_aug + length_aug]
                    / self.norm_factor
                )
...
class GetAcc(GetLoss):
    ...
    def __call__(self, data, mA, mB):
        ...
        for i, length_noaug in enumerate(self.style_seq):
            ...
            if length_noaug:
                pred_label = data[mA]["label"][
                    start_id_noaug : start_id_noaug + length_noaug
                ]
...
def reconstruct_motion(
    ...
    X[:, 12:, 0, [0, 2]] = (
        X[:, 12:, 0, [0, 2]]
        + (X[:, target_id : target_id + 1, 0, [0, 2]] - X[:, 10:11, 0, [0, 2]])
        * location_x
    )
    ...
    def get_gt_pose(n_past, target_id):
        trans_gt_global_poses, trans_gt_global_quats = (
            global_poses[:, n_past:target_id, ...],
            global_quats[:, n_past:target_id, ...],
        )
    ...
    return (
        data,
        global_poses[:, n_past - 1 : n_past, :, :],
        global_quats[:, n_past - 1 : n_past, :, :],
    )
...
def save_data_to_binary(path, data):
    ...
    for condition in data.keys():
        ...
        for method in data[condition].keys():
            ...
            data[condition][method]["pos"] = data[condition][method]["pos"][
                :, :, 0:1, :
            ]  # b,t,1,3
    pickle.dump(data, f)
    for condition in data.keys():
...
def load_data_from_binary(path, skeleton: Skeleton):
    ...
    for condition in data.keys():
        for method in data[condition].keys():
            ...
            data[condition][method]["pos"] = skeleton.global_rot_to_global_pos(
                rot, offsets, pos
            )
    f.close()
    return data
    ...
    for metric_name in metrics.keys():
        if metric_name != "NPSS":
            ...
            for method_name in all_methods:
                method_metric = [
                    metrics[metric_name][condition][method_name]
...
def calculate_stat(
    ...
    if load_from_dict:
        ...
        with torch.no_grad():
            for condition in conditions:
                print(
                    "Reconstructing motions for condition: length={}, x={}, t={} ...".format(
                        condition.length, condition.x, condition.t
                    )
                )
                start_id = 0
                ...
                for batch in dataLoader:
                    ...
                    tar_pos, tar_quat = BatchRotateYCenterXZ().forward(
                        tar_gp, tar_gq, 10
                    )
                    ...
                    data, last_pos, last_rot = reconstruct_motion(
                        models,
                        function,
                        hip_pos.cuda(),
                        local_quat.cuda(),
                    ...
                    if start_id == 0:
                        for method in data.keys():
                            for prop in data[method].keys():
                                output[method][prop] = torch.empty(
                                    (data_size,) + data[method][prop].shape[1:]
                                )
                    for method in data.keys():
                        batch_size = data[method]["pos"].shape[0]
                        props = list(data[method].keys())
                        for prop in props:
                            output[method][prop][start_id : start_id + batch_size] = (
                                data[method][prop]
                            )
    ...
    for condition in conditions:
        if condition.length == 40:
            print(
                "Reconstructing latent for condition: length={}, x={}, t={} ...".format(
                    condition.length, condition.x, condition.t
                )
            )
            t = time.time()
            ...
            for method in data.keys():
                ...
                while start_id < length:
                    ...
                    rot, pos = (
                        data[method]["rot"][start_id : start_id + window],
                        data[method]["pos"][start_id : start_id + window],
                    )
                    ...
                    glb_vel, glb_pos, glb_rot, root_rotation = (
                        BatchProcessDatav2().forward(rot, pos)
                    )
    ...
    for metric_key in benchmarks.keys():
        ...
        if metric_key == "Diversity":
            for condition in data.keys():
                ...
                metrics[metric_key][condition]["div_test"] = metric(
                    data[condition], "div_test"
                )
        else:
            for condition in data.keys():
                print("condition : {}".format(condition))
                ...
                for method in data[condition].keys():
                    if method == "div_test":
                        continue
                    metrics[metric_key][condition][method] = metric(
                        data[condition], method, "gt"
                    )
    return metrics
...
def benchmarks(args, load_stat=False, data_name=None):
    ...
    style_seqs = [
        len(style_loader.all_motions[style_key]["motion"]["quats"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="./results/Transitionv2_style100/myResults/117/m_save_model_205",
    )
