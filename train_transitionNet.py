...
def create_common_states(prefix: str):
    ...
    parser.add_argument(
        "--moe_model",
        type=str,
        default="./results/StyleVAE2_style100/myResults/55/m_save_model_332",
    )
    ...
    tb_logger = pl.loggers.TensorBoardLogger(
        save_dir="tensorboard_logs/", name=log_name, version=None
    )
...
def training_style100_phase():
    ...
    if args.test == False:
        ...
        data_module = StyleVAE_DataModule(
            ...
            batch_size=32,
            mirror=0.0,
        )  # when apply phase, should avoid mirror
        ...
        if args.epoch == "last":
            ...
            for dir in dirs:
                ...
                if len(out) > 0:
                    ...
                    print(check_file)
                    break
        model = TransitionNet_phase.load_from_checkpoint(
            check_file,
            moe_decoder=moe_net,
            pose_channels=9,
            phase_dim=phase_dim,
