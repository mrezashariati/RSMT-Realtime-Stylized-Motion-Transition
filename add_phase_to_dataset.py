...
def add_phase_to_100Style(info):
    phase_processor = PhaseMotionStyle100Processor(
        info["window"], info["dt"], info["model_path"]
    )
    ...
    def add_phase(motions):
        for style in motions.keys():
            ...
            for content in motions[style].keys():
                print(content)
                motions[style][content] = phase_processor(
                    motions[style][content], style_loader.skeleton
                )
        return motions
