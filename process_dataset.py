...
import numpy as np
import random
from src.Datasets.Style100Processor import (
    StyleLoader,
    Swap100StyJoints,
    bvh_to_binary,
    save_skeleton,
...
from src.Datasets.BaseLoader import (
    BasedDataProcessor,
    BasedLoader,
    DataSetType,
    MotionDataLoader,
...
class TransitionProcessor(BasedDataProcessor):
    ...
    def calculate_statistic(self, local_pos, local_rot):
        pos_mean, pos_std = self.calculate_pos_statistic(
            local_pos[:, :, 1:, :].cpu().numpy()
        )
        vel_mean, vel_std = self.calculate_pos_statistic(
            (local_pos[:, 1:, 1:, :] - local_pos[:, :-1, 1:, :]).cpu().numpy()
        )
        hipv_mean, hipv_std = self.calculate_pos_statistic(
            (local_pos[:, 1:, 0:1, :] - local_pos[:, :-1, 0:1, :]).cpu().numpy()
        )
        rot_mean, rot_std = self.calculate_rotation_mean(
            local_rot[:, :, 1:, :].cpu().numpy()
        )
        rotv_mean, rotv_std = self.calculate_rotation_mean(
            (local_rot[:, 1:, 1:, :] - local_rot[:, :-1, 1:, :]).cpu().numpy()
        )
        hipr_mean, hipr_std = self.calculate_rotation_mean(
            local_rot[:, :, 0:1, :].cpu().numpy()
        )
        hiprv_mean, hiprv_std = self.calculate_rotation_mean(
            (local_rot[:, 1:, 0:1, :] - local_rot[:, :-1, 0:1, :]).cpu().numpy()
        )

        return {
            "pos_stat": [pos_mean, pos_std],
            "rot_stat": [rot_mean, rot_std],
            "vel_stat": [vel_mean, vel_std],
            "rotv_stat": [rotv_mean, rotv_std],
            "hipv_stat": [hipv_mean, hipv_std],
            "hipr_stat": [hipr_mean, hipr_std],
            "hiprv_stat": [hiprv_mean, hiprv_std],
        }
...
def read_style_bvh(style, content, clip=None):
    ...
    anim = BVH.read_bvh(
        os.path.join("MotionData/100STYLE/", style, style + "_" + content + ".bvh"),
        remove_joints=swap_joints,
    )
    if clip != None:
...
def processTransitionPhaseDatasetForStyle100(window, overlap):
    ...
    def split_window(motions):
        ...
        for style in motions.keys():
            for content in motions[style].keys():
                motions[style][content] = window_loader.append_dicts(
                    motions[style][content]
                )
        return motions
...
def processDeepPhaseForStyle100(window, overlap):
    ...
    style_loader = StyleLoader()
    window_loader = mBaseLoader.WindowBasedLoader(window, overlap, 1)
    processor = DeepPhaseProcessor(1.0 / 30)
    style_loader.setup(window_loader, processor)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="./results/deephase_sty/myResults/31/epoch=161-step=383778-v1.ckpt",
    )
    ...
    if args.preprocess == True:
        ...
        print("\nConvert down\n")
        print(
            "Divide the dataset to train set and test set, and then argument datasets."
        )
        ...
        style100_info = {"model_path": args.model_path, "dt": 1.0 / 30, "window": 61}
