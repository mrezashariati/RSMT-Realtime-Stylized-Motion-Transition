...
def test_model():
    dict = {}
    dict["limit_train_batches"] = 1.0
    dict["limit_val_batches"] = 1.0
    return dict
...
def create_common_states(prefix: str):
    ...
    tb_logger = pl.loggers.TensorBoardLogger(
        save_dir="tensorboard_logs/", name=log_name, version=version
    )
...
def training_style100():
    args, trainer_dict, resume_from_checkpoint, ckpt_path = create_common_states(
        "deephase_sty"
    )
    """Create the model"""
    frequency = 30
    ...
    batch_size = 32
    data_module = Style100DataModule(
        batch_size=batch_size,
        shuffle=True,
        data_loader=style_loader,
        window_size=window,
    )
    model = DeepPhaseNet(
        args.n_phases,
        ...
        1.0 / frequency,
        batch_size=batch_size,
    )  # or model = pl.LightningModule().load_from_checkpoint(PATH)
    if args.test == False:
        if args.dev_run:
            trainer = Trainer(
                **trainer_dict,
            ...
            trainer = Trainer(
                **trainer_dict,
                max_epochs=500,
                **select_gpu_par(),
                log_every_n_steps=50,  # limit_train_batches=0.1,
                flush_logs_every_n_steps=500,
...
def readBVH(filename, dataset_property):
    ...
    return BVH.read_bvh(
        ...
        remove_joints=remove_joints,
        Tpose=-1,
        remove_gap=dataset_property["remove_gap"],
    )
