...
def find_level_recursive(map, parent, node, level_joint, level_joint_parent, level):
    ...
    for child_id in map[node]["children"]:
        find_level_recursive(
            map, node, child_id, level_joint, level_joint_parent, level + 1
        )
...
class Skeleton(object):
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.bone_pair_indices == other.bone_pair_indices
        )
    ...
    def __build_level_map(self):
        ...
        find_level_recursive(
            self.hierarchy, -1, self.root, level_joint, level_joint_parent, 0
        )
    ...
    def forward_kinematics(
        ...
        gp, gq = fk.forward_kinematics_quats(
            local_rotations, local_offsets, hip_positions, self.parents
        )
        return gp, gq
    ...
    def inverse_kinematics(
        self,
        joints_global_rotations: torch.Tensor,
        joints_global_positions: torch.Tensor,
        output_other_dim=False,
    ):
        lp, lq = ik.inverse_kinematics_quats(
            joints_global_rotations, joints_global_positions, self.parents
        )
        if output_other_dim:
            return lp, lq
        else:
    ...
    def global_rot_to_global_pos(self, global_rot, offsets, hip):
        ...
        gp = torch.empty(
            pos_shape, dtype=global_rot.dtype, device=global_rot.device
        )  # [hip]
        gp[..., 0:1, :] = hip[..., 0:1, :]
        offset = trans.quaternion_apply(
            global_rot[..., self.parents[1:], :], offsets[..., 1:, :]
        )
        ...
        for level in range(1, len(self._level_joints)):
            ...
            gp[..., local_bone_indices, :] = (
                offset[..., local_bone_indices_1, :] + gp[..., parent_bone_indices, :]
            )
    ...
    def inverse_pos_to_rot(
        self, joints_global_rotations, joints_global_positions, offsets, tangent
    ):
        edge_len = torch.norm(offsets[:, 1:], dim=-1, keepdim=True)
        joints_global_positions = ik.scale_pos_to_bonelen(
            joints_global_positions,
            edge_len,
            self._level_joints,
            self._level_joints_parents,
        )
        parent_rot = self.pos_to_parent_rot(
            joints_global_rotations, joints_global_positions, offsets, tangent
        )
    ...
    def pos_to_parent_rot(
        self,
        joints_global_rotations: torch.Tensor,
        joints_global_positions: torch.Tensor,
        local_offsets: torch.Tensor,
        secondary_offsets: torch.Tensor,
    ):
        return ik.change_pos_to_rot(
            joints_global_rotations,
            joints_global_positions,
            local_offsets,
            secondary_offsets,
            self.parents,
...
class Anim(object):
    ...
    def __init__(
        ...
        if remove_joints != None:
            self.quats, self.offsets, self.parents, self.names = remove_joints(
                self.quats, self.offsets, self.parents, self.names
            )

        self.skeleton = Skeleton(self.parents, self.names, self.offsets)
        if remove_gap > 0.0:

            gp, gq = self.skeleton.forward_kinematics(
                torch.from_numpy(self.quats),
                torch.from_numpy(self.offsets),
                torch.from_numpy(self.hip_pos),
            )
        ...
        self.secondary_offsets = find_secondary_axis(
            torch.from_numpy(self.offsets)
        ).numpy()
...
def read_bvh(
    ...
    rotations = remove_quat_discontinuities(torch.from_numpy(rotations).unsqueeze(0))[
        0
    ].numpy()
