...
def quat_to_or6D(quat):
    ...
    return pytorch3d.transforms.matrix_to_rotation_6d(
...
def or6d_to_quat(mat):
    ...
    return pytorch3d.transforms.matrix_to_quaternion(
        pytorch3d.transforms.rotation_6d_to_matrix(mat)
    )
...
def from_to_1_0_0(v_from):
    """v_from: B,3"""
    v_to_unit = torch.tensor(
        np.array([[1, 0, 0]]), dtype=v_from.dtype, device=v_from.device
    ).expand(v_from.shape)
    y = torch.tensor(
        np.array([[0, 1, 0]]), dtype=v_from.dtype, device=v_from.device
    ).expand(v_from.shape)
    z_to_unit = torch.tensor(
        np.array([[0, 0, 1]]), dtype=v_from.dtype, device=v_from.device
    ).expand(v_from.shape)
