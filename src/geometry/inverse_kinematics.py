...
def invert_transform_hierarchy(
    ...
    for level in range(len(level_transforms) - 1, 0, -1):
        ...
        local_matrix = torch.matmul(
            torch.inverse(parent_level_transforms), local_level_transforms
        )
        local_transforms[..., local_bone_indices, :, :] = local_matrix.type_as(
            local_transforms
        )
...
def inverse_kinematics(
    ...
    global_transforms = get_transform_matrix(
        translations=global_offsets, rotations=global_rotations
    )
    # used to store local transforms
    local_transforms = global_transforms.clone()
...
def inverse_kinematics_only_quats(joints_global_rotations: torch.Tensor, parents):
    ...
    joint_local_quats = torch.cat(
        (
            joint_local_quats[..., :1, :],
            quaternion_multiply(
                invert_global_rotations[..., parents[1:], :],
                joint_local_quats[..., 1:, :],
            ),
        ),
        dim=-2,
...
def inverse_kinematics_quats(
    joints_global_rotations: torch.Tensor, joints_global_positions, parents
):
    """
    Performs differentiable inverse kinematics with quaternion-based rotations
    :param joints_global_rotations: tensor of global quaternions of shape (..., J, 4)
    ...
    joint_local_pos = torch.cat(
        [
            joints_global_positions[..., :1, :],
            quaternion_apply(
                invert_global_rotations[..., parents[1:], :],
                joints_global_positions[..., 1:, :]
                - joints_global_positions[..., parents[1:], :],
            ),
        ],
        dim=-2,
    )
...
def scale_pos_to_bonelen(
    glb_pos: torch.Tensor,
    edge_len: torch.Tensor,
    level_joints: List[List[int]],
    level_joint_parents: List[List[int]],
):
    edge_len = edge_len.unsqueeze(1).expand(
        glb_pos.shape[:2] + (glb_pos.shape[2] - 1, 3)
    )
...
def change_pos_to_rot(
    ...
    joint_offset = (
        joints_global_positions[..., 1:, :]
        - joints_global_positions[..., parent[1:], :]
    )
    ...
    secondary_offsets = (
        secondary_offsets[..., 1:, :].unsqueeze(-3).expand(joint_offset.shape)
    )
    ...
    init_ore = quaternion_apply(
        joint_local_quats[..., parent[1:], :], secondary_offsets
    )
