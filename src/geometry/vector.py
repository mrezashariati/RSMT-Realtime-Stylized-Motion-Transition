...
def find_secondary_axis(v):
    ...
    refx = torch.tensor(np.array([[1, 0, 0]]), dtype=v.dtype, device=v.device).expand(
        v.shape
    )
    refy = torch.tensor(np.array([[0, 1, 0]]), dtype=v.dtype, device=v.device).expand(
        v.shape
    )
...
def normalize_vector(v, return_mag=False):
    ...
    )  # torch.sqrt(v.pow(2).sum(-1,keepdim=True))#+eps
