...
def bvh_to_binary():
    ...
    content_range = {
        name: extractSeqRange(frame_cuts[name + "_START"], frame_cuts[name + "_STOP"])
...
def save_skeleton():
    root_dir = "./MotionData/100STYLE/"
    anim = read_bvh(
        root_dir + "Aeroplane/Aeroplane_BR.bvh", remove_joints=Swap100StyJoints()
    )
...
class StyleLoader:
    ...
    def augment_dataset(self):
        ...
        mirror = BatchMirror(self.skeleton, 1.0)
        scale = TemporalScale(1.0)
        ...
        def augment_motions(motions):
            for style in motions.keys():
                ...
                for content in content_keys:
                    ...
                    offsets = (
                        torch.from_numpy(seq["offsets"]).unsqueeze(0).float().cuda()
                    )
                    ...
                    mirror_hips, mirror_quats = (
                        mirror_hips.squeeze(0).cpu().numpy(),
                        mirror_quats.squeeze(0).cpu().numpy(),
                    )
                    motions[style]["mr_" + content] = {
                        "quats": mirror_quats,
                        "offsets": seq["offsets"],
                        "hips": mirror_hips,
                    }
                    ...
                    sc_hips, sc_quats = (
                        sc_hips.squeeze(0).cpu().numpy(),
                        sc_quats.squeeze(0).cpu().numpy(),
                    )
                    motions[style]["sca_" + content] = {
                        "quats": sc_quats,
                        "offsets": seq["offsets"],
                        "hips": sc_hips,
                    }
        ...
        for style in self.style_names[:-10]:
            ...
            for content in self.all_motions[style].keys():
                ...
                if length > 2000:
                    ...
                    self.train_motions[style][content]["quats"] = seq["quats"][
                        :-test_length
                    ]
                    self.train_motions[style][content]["offsets"] = seq[
                        "offsets"
                    ]  # [:-test_length]
                    self.train_motions[style][content]["hips"] = seq["hips"][
                        :-test_length
                    ]
                    self.test_motions[style][content] = {}
                    self.test_motions[style][content]["quats"] = seq["quats"][
                        -test_length:
                    ]
                    self.test_motions[style][content]["offsets"] = seq[
                        "offsets"
                    ]  # [-test_length:]
                    self.test_motions[style][content]["hips"] = seq["hips"][
                        -test_length:
                    ]
                else:
    ...
    def _process_dataset(self, motions):
        ...
        for style_name in motions.keys():
            for content_name in motions[style_name]:
                ...
                off, hip, quat = self.loader.load_data(
                    dict["offsets"], dict["hips"], dict["quats"]
                )
        ...
        train_set = self.processor(
            {"offsets": o, "hip_pos": h, "quats": q}, self.skeleton, self
        )
