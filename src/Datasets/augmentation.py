...
def find_Yrotation_to_align_with_Xplus(q):
    ...
    q = q.reshape(-1, 4)
    mask = torch.tensor(
        np.array([[1.0, 0.0, 1.0]]), dtype=q.dtype, device=q.device
    ).expand(q.shape[0], -1)
    ref_vector = torch.tensor(
        np.array([[1.0, 0.0, 0.0]]), dtype=q.dtype, device=q.device
    ).expand(q.shape[0], -1)
...
def angle_between_quats_along_y(from_quat: torch.Tensor, to_quat: torch.Tensor):
    ...
    initial_direction = torch.tensor(
        np.array([[0.0, 0.0, 1.0]]), dtype=torch.float, device=from_quat.device
    ).expand(from_quat.shape[:-1] + (3,))
...
class TemporalScale(torch.nn.Module):
    ...
    def forward(self, hip_pos, quat):
        ...
        new_hip_pos = torch.zeros(
            (batch_size, new_delta_t, 1, 3), device=hip_pos.device, dtype=hip_pos.dtype
        )
        new_quat = torch.zeros(
            (batch_size, new_delta_t, num_joints, 4),
            device=hip_pos.device,
            dtype=hip_pos.dtype,
        )
        """scale"""
        ...
        neighbor_frame = torch.where(
            neighbor_frame >= quat.shape[1], neighbor_frame - 1, neighbor_frame
        )
        weight = start_frame - ori_frame_step + 1
        weight = weight.view(1, -1, 1, 1)

        new_quat[:, idx, :, :] = slerp(
            quat[:, start_frame, :, :], quat[:, neighbor_frame, :, :], 1 - weight
        )
        new_hip_pos[:, idx, :, :] = (
            hip_pos[:, start_frame, :, :] * weight
            + (1 - weight) * hip_pos[:, neighbor_frame, :, :]
        )
...
class BatchMirror(torch.nn.Module):
    ...
    def forward(self, global_pos, global_rot):
        if torch.rand(1)[0] < self.mirror_probability:
            ...
            mirrored_positions, mirrored_rotations = self.mirroring.forward(
...
class Mirror(torch.nn.Module):
    def __init__(self, skeleton: Skeleton, dtype: torch.dtype):
        ...
        self.register_buffer(
            "swap_index_list", torch.LongTensor(skeleton.bone_pair_indices)
        )

    def forward(
        self, joint_positions: torch.Tensor = None, joint_rotations: torch.Tensor = None
    ):  # t position in global space
        ...
        if joint_positions is None:
            ...
            new_positions = torch.matmul(
                self.reflection_matrix, new_positions.permute(1, 2, 0)
            ).permute(2, 0, 1)
