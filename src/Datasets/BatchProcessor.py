...
class BatchProcessDatav2(torch.nn.Module):
    ...
    def forward(self, glb_rot, glb_pos):
        ...
        ref_vector = torch.cat(
            (-dir[..., 2:3], torch.zeros_like(dir[..., 1:2]), dir[..., 0:1]), dim=-1
        )
        root_rotation = from_to_1_0_0(ref_vector)
        glb_vel = quaternion_apply(
            root_rotation[:, :-1], glb_pos[:, 1:] - glb_pos[:, :-1]
        )
...
class BatchUnProcessDatav2(torch.nn.Module):
    ...
    def forward(self, glb_pos, glb_rot, glb_vel, root_rotation):
        ...
        out_pos = torch.empty(
            size=glb_rot.shape[:-1] + (3,), dtype=glb_pos.dtype, device=glb_pos.device
        )
...
class BatchProcessData(torch.nn.Module):  # (B,T,J,dim)
    ...
    def forward(self, global_rotations, global_positions):
        v1 = global_positions[..., 5:6, :] - global_positions[..., 1:2, :]
        v2 = torch.tensor(
        ...
        local_velocities = quaternion_apply(
            root_rotation[:, :-1], (global_positions[:, 1:] - global_positions[:, :-1])
        )
        ...
        if torch.isnan(local_rotations).any():
            print("6D")
            assert False
        root_global_velocity_XZ = (
            global_positions[:, 1:, 0:1] - global_positions[:, :-1, 0:1]
        )
        root_global_velocity_XZ[..., 1] = 0
        root_velocity = quaternion_apply(
            root_rotation[:, :-1, :], (root_global_velocity_XZ)
        )
        # root_rvelocity = angle_between_quats_along_y(global_rotations[:,:-1, 0:1, :], global_rotations[:,1:, 0:1, :])
        root_rvelocity = angle_between_quats_along_y(
            root_rotation[:, 1:, 0:1, :], root_rotation[:, :-1, 0:1, :]
        )
        dict = {
            "root_rotation": root_rotation,
            "local_pos": local_positions[:, :, ...],
            "local_vel": local_velocities,
            "local_rot": local_rotations[:, :, ...],
            "root_vel": root_velocity[..., [0, 2]],
            "root_rvel": root_rvelocity.unsqueeze(-1),
        }
...
class UnProcessData(torch.nn.Module):
    ...
    def get_global_xz(self, global_hip_rot, hip_velocity):
        root_velocity3D = torch.zeros(
            size=(hip_velocity.shape[0], hip_velocity.shape[1], 1, 3),
            device=global_hip_rot.device,
            dtype=hip_velocity.dtype,
        )
        ...
        for i in range(1, (root_velocity3D.shape[1] + 1)):
            root_pos_XZ[:, i, 0, :] = (
                root_pos_XZ[:, i - 1, 0, :] + root_velocity3D[:, i - 1, 0, :]
            )
        return root_pos_XZ
