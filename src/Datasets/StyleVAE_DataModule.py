...
class Style100Dataset_phase(torch.utils.data.Dataset):
    ...
    def get_style_batch(self, style, style_id, batch_size):
        ...
        for i in range(len(sub_motions)):
            dict = {
                key: torch.from_numpy(sub_motions[i][3][key]).unsqueeze(0).cuda()
                for key in sub_motions[i][3].keys()
            }
            sub_motions[i] = [
                torch.from_numpy(sub_motions[i][j]).unsqueeze(0).cuda()
                for j in range(3)
            ] + [dict]
    ...
    def expand_(self):
        for style in self.dataset:
            ...
            self.dataset[style] = [
                (q[i], o[i], h[i], {"A": A[i], "S": S[i], "B": B[i], "F": F[i]})
    ...
    def shuffle_(self):
        ...
        for style_id, style in enumerate(self.dataset):
            ...
            self.expand_dataset["sty"] += [
                self.style_ids[style_id] for i in range(self.sizes[style_id])
            ]

    def __getitem__(self, item):

        return (
            self.expand_dataset["data"][item],
            self.expand_dataset["sty"][item],
            self.is_train,
        )
...
class StyleVAE_DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: [str] = None) -> None:
        ...
        self.train_set = Style100Dataset_phase(
            self.loader.train_motions, self.batch_size, True
        )
        self.test_set = Style100Dataset_phase(
            self.loader.test_motions, self.batch_size, False, keep_equal=False
        )
        self.val_set = self.test_set
        if self.use_sty:
            self.loader.load_dataset(self.style_file_name)
            self.train_set_sty = Style100Dataset_phase(
                self.loader.train_motions, self.batch_size, True, keep_equal=False
            )
            self.test_set_sty = Style100Dataset_phase(
                self.loader.test_motions, 4, False, keep_equal=False
            )
            self.val_set = self.test_set
    ...
    def transfer_mannual(
        ...
        if "con" in batch:
            quat = torch.cat(
                (get_data(batch["con"], 0), get_data(batch["sty"], 0)), dim=0
            )
            hip_pos = torch.cat(
                (get_data(batch["con"], 2), get_data(batch["sty"], 2)), dim=0
            )
            offsets = torch.cat(
                (get_data(batch["con"], 1), get_data(batch["sty"], 1)), dim=0
            )
            sty = [batch["con"][1], batch["sty"][1]]
            if use_phase:
                A = torch.cat(
                    (get_phase(batch["con"], "A"), get_phase(batch["sty"], "A")), dim=0
                )
                S = torch.cat(
                    (get_phase(batch["con"], "S"), get_phase(batch["sty"], "S")), dim=0
                )
                A = A / 0.1
        ...
        if use_sty:
            ...
            style_gp, style_gq = self.skeleton.forward_kinematics(
                style_quat, style_offsets, style_hip
            )
            if (
                is_train == False
            ):  # the test set is too small so there is not enough batch
                style_gp = (
                    style_gp.unsqueeze(0).expand((8,) + style_gp.shape).flatten(0, 1)
                )
                style_gq = (
                    style_gq.unsqueeze(0).expand((8,) + style_gq.shape).flatten(0, 1)
                )
        else:
    ...
    def on_after_batch_transfer(self, batch, dataloader_idx: int):
        return self.transfer_mannual(
            batch, dataloader_idx, self.use_phase, use_sty=self.use_sty
        )
