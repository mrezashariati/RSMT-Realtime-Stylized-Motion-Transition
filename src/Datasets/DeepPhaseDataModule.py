...
class DeepPhaseProcessorv2(BasedDataProcessor):
    ...
    def transform_single(self, offsets, hip_pos, local_quat, skeleton):
        gp, gq = self.gpu_fk(offsets, hip_pos, local_quat, skeleton)
        glb_vel, glb_pos, glb_rot, root_rotatioin = self.process.forward(
            gq.unsqueeze(0), gp.unsqueeze(0)
        )
    ...
    def __call__(self, dict, skeleton, motionDataLoader):
        ...
        for i in range(len(offsets)):
            N, J, D = quats[i].shape
            dict["gv"].append(
                self.transform_single(offsets[i], hip_pos[i], quats[i], skeleton)
            )
...
class Style100DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: [str] = None):
        ...
        if use_gv:
            ...
            self.test_set = DeephaseDataSet(
                self.data_loader.test_dict["pos"], self.window
            )
            self.train_set = DeephaseDataSet(
                self.data_loader.train_dict["pos"], self.window
            )
            self.scale = 1.0
