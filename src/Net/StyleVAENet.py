...
class MoeGateDecoder(nn.Module):
    def __init__(
        ...
        self.linears = nn.ModuleList(
            [
                MultipleExpertsLinear(condition_size + latent_size, 512, num_experts),
                MultipleExpertsLinear(512 + latent_size, 512, num_experts),
            ]
        )
...
class SingleVAEOperator:
    ...
    def shift_encode_two_frame(
        this, self, last_v, last_rots, nxt_v, nxt_rots, contact, last_phase, next_phase
    ):
        co_input = torch.cat(
            (
                last_v,
                ...
                nxt_v,
                nxt_rots,
                last_phase.flatten(-2, -1),
                next_phase.flatten(-2, -1),
            ),
            dim=-1,
        )
...
class EmbeddingTwoFrameEncoder(nn.Module):
    def __init__(self, num_joints, num_pos_joints, latent_dims, phase_dims):
        ...
        self.linears = nn.ModuleList(
            [nn.Linear(in_channels, 512), nn.Linear(512, 512), nn.Linear(512, 512)]
        )
        self.act = nn.ModuleList([nn.ELU(), nn.ELU(), nn.ELU()])
...
class StyleVAENet(pl.LightningModule):
    ...
    def __init__(
        ...
        self.vae_op = (
            SingleVAEOperator() if net_mode == VAEMode.SINGLE else MultiVAEOperator()
        )

        self.batch_processor = BatchProcessDatav2()
        self.embedding_encoder = EmbeddingTwoFrameEncoder(
            skeleton.num_joints, len(self.pos_rep_idx), latent_size, phase_dim
        )
        self.decoder = MoeGateDecoder(
            style_level_dim,
            skeleton.num_joints,
    ...
    def shift_running(
        self, local_pos, local_rots, phases, As, Ss, contacts, style_code
    ):
        """pose: N,T,J,9"""
        """hip: N,T,3"""
        """phases: N,T,M"""
        ...
        output_mu = torch.empty(
            size=(N, T - 2, self.latent_size), device=local_rots.device
        )
        ...
        for t in range(1, T - 1):  # slice+1: we discard the first frame
            ...
            next_rel_pos = (local_pos[:, t + 1] - local_pos[:, t + 1, 0:1]).flatten(
                -2, -1
            )
            ...
            condition_no_style = torch.cat(
                (last_rel_pos, last_l_v, hip_l_v, last_l_rot, hip_l_r), dim=-1
            )
    ...
    def phase_loss(self, gt_phase, gt_A, gt_F, phase, A, F, sphase):
        amp_scale = 1.0
        loss = {
            "phase": self.mse_loss(gt_phase * amp_scale, phase * amp_scale),
            "A": self.mse_loss(gt_A * amp_scale, A * amp_scale),
            "F": self.mse_loss(gt_F, F),
            "slerp_phase": self.mse_loss(gt_phase * amp_scale, sphase * amp_scale),
        }
        return loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.stage = "training"
        self.scheduled_prob = self.common_operator.get_progress(
            self, target_epoch=50, start_epoch=0
        )
    ...
    def configure_optimizers(self):
        ...
        params = weight_decay("decoder", lr, 0) + weight_decay(
            "embedding_encoder", lr, 0
        )
...
class Application(nn.Module):
    ...
    def draw_foot_vel(self, pos, pred_pos):
        ...
        def draw(pos, ax, key_joints):
            foot_vel = (
                ((pos[:, 1:, key_joints] - pos[:, :-1, key_joints]) ** 2)
                .sum(dim=-1)
                .sqrt()
            )
            ax.plot(foot_vel[0, :].cpu())

        fig, ax = plt.subplots(2, 2, figsize=(2 * 1.5, 2 * 1.0))
        joints = [17, 18, 21, 22]
        ax[0, 0].set_xlabel("3")
    ...
    def forward(self, seed, encoding=True):
        ...
        with torch.no_grad():
            ...
            def draw_projection(pred_mu):
                ...
                proj = proj[:, :2]
                c = np.arange(0, proj.shape[0], step=1.0)
                proj = proj.cpu()
