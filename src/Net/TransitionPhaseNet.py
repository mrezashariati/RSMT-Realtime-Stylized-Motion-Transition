...
def eval_sample(
    ...
    if model.predict_phase == True:
        pred_pos, pred_rot, pred_phase, _, _ = model.shift_running(
            gp.cuda(),
            loc_rot.cuda(),
            phases.cuda(),
            A.cuda(),
    ...
    GX = skeleton.global_rot_to_global_pos(GQ, offsets, pred_pos[:, :, 0:1, :]).flatten(
        -2, -1
    )
...
class StateEncoder(nn.Module):
    def __init__(self, inchannels):
        ...
        self.layers = torch.nn.ModuleList(
            [torch.nn.Linear(inchannels, 512), torch.nn.Linear(512, 256)]
        )
...
class MoeStylePhasePredictor(nn.Module):
    def __init__(self, rnn_size, phase_dim, num_experts):
        ...
        self.linears = nn.ModuleList(
            [
                nn.Linear(rnn_size + phase_dim * 2 + 512, rnn_size),
    ...
    def forward(self, fs, condition, phase, offset_latent, first):
        ...
        out = self.mlp(x)
        phase, hip, latent = (
            out[:, : self.phase_dims * 4],
            out[:, self.phase_dims * 4 : self.phase_dims * 4 + 9],
            out[:, self.phase_dims * 4 + 9 :],
        )
...
class PredictInitialState(nn.Module):
    def __init__(self):
        ...
        self.phase_op = PhaseOperator(1 / 30.0)
...
class TransitionNet_phase(pl.LightningModule):
    ...
    def __init__(
        ...
        self.pos_rep_idx = [
            idx
            for idx in np.arange(0, skeleton.num_joints)
            if idx not in self.rot_rep_idx
        ]
        self.mode = mode
        self.test = True
        ...
        register_numpy = lambda x, s: self.register_buffer(
            s, torch.from_numpy(x).float()
        )
        register_scalar = lambda x, s: self.register_buffer(
        ...
        self.state_input_size = (num_joints) * 6 + len(
            self.pos_rep_idx
        ) * 6  # +phase_dim*2# c_t, root velocity, q_t
        self.offset_input_size = (num_joints) * 6 + len(
            self.pos_rep_idx
        ) * 3  # offset from target, root offset, pose offset
        self.target_input_size = (num_joints) * 6 + len(
            self.pos_rep_idx
        ) * 6  # + phase_dim*2# q_target
    ...
    def target_encoding(self, target_rots, target_pos, target_v):
        return self.target_encoder(
            torch.cat(
                (
                    target_pos.flatten(-2, -1),
                    target_v.flatten(-2, -1),
                    target_rots.flatten(-2, -1),
                ),
                dim=-1,
            )
        )
    ...
    def state_encoding(self, pos, vel, rot):
        return self.state_encoder(
            torch.cat(
                (pos.flatten(-2, -1), vel.flatten(-2, -1), rot.flatten(-2, -1)), dim=-1
            )
        )

    def offset_encoding(self, pos, rot):
        return self.offset_encoder(
            torch.cat((pos.flatten(-2, -1), rot.flatten(-2, -1)), dim=-1)
        )

    def regu_pose(self, pos, edge_len, rot):
        import src.geometry.inverse_kinematics as ik

        pos = ik.scale_pos_to_bonelen(
            pos,
            edge_len,
            self.skeleton._level_joints,
            self.skeleton._level_joints_parents,
        )
    ...
    def shift_running(
        ...
        output_phase = torch.empty(
            size=(N, length, phases.shape[-2], 2), device=phases.device
        )
        output_sphase = torch.empty(
            size=(N, length, phases.shape[-2], 2), device=phases.device
        )
        output_A = torch.empty(
            size=(N, length, phases.shape[-2], 1), device=phases.device
        )
        ...
        offset_t = torch.scalar_tensor(
            start_id + length - 1, dtype=torch.int32, device=device
        )
        ...
        target_g_pos, target_g_rots = (
            local_pos[:, target_id - 1, :],
            local_rots[:, target_id - 1, :],
        )
        ...
        target_latent = self.target_encoding(
            target_g_rots, target_g_pos - target_g_pos[:, 0:1], target_g_v
        )
        ...
        if hasattr(self, "predict_phase") == False or self.predict_phase == False:
            for i in range(1, start_id - 1):
                ...
                state_latent = self.state_encoding(
                    last_l_pos - last_l_pos[:, 0:1], last_l_v, last_l_rot
                )
                offset_latent = self.offset_encoding(offset_pos, offset_rot)
                state_latent = self.embedding_style(
                    style_code, state_latent, None, encode_first
                )
                latent, h_target = concat(
                    state_latent,
                    offset_latent,
                    target_latent,
                    self.embedding,
            ...
            last_l_v, target_l_v = local_pos[:, 1 : start_id - 1] - local_pos[
                :, 0 : start_id - 2
            ], target_g_v.repeat(1, start_id - 2, 1, 1)
            last_l_pos, target_l_pos = local_pos[
                :, 1 : start_id - 1
            ], target_g_pos.unsqueeze(1).repeat(1, start_id - 2, 1, 1)
            last_l_rot, target_l_rot = local_rots[
                :, 1 : start_id - 1
            ], target_g_rots.unsqueeze(1).repeat(1, start_id - 2, 1, 1)
            offset_pos = target_l_pos - last_l_pos
            offset_rot = target_l_rot - last_l_rot
            ...
            offset_latent = self.offset_encoding(offset_pos, offset_rot)
            style_code[0] = (
                style_code[0].unsqueeze(1).repeat(1, start_id - 2, 1, 1).flatten(0, 1)
            )
            state_latent = self.embedding_style(
                style_code, state_latent, None, first=True
            )

            target_latent = (
                target_latent.unsqueeze(1).repeat(1, start_id - 2, 1).flatten(0, 1)
            )
            ## recover
            ...
            last_g_pos, last_g_rot = (
                local_pos[:, start_id - 1],
                local_rots[:, start_id - 1],
            )
        ...
        for i in range(start_id - 1, start_id - 1 + length):
            ...
            input_clip = hn
            pred_phase, pred_A, pred_F, hip_l_v, hip_l_rv, latent = (
                self.phase_predictor(
                    style_code, input_clip, last_phase, h_target, first
                )
            )
            hip_l_r = hip_l_rv + last_l_rot[:, 0]
            condition_no_style = torch.cat(
                (
                    (last_l_pos - last_l_pos[:, 0:1]).flatten(-2, -1),
                    last_l_v.flatten(-2, -1),
                    hip_l_v,
                    last_l_rot.flatten(-2, -1),
                    hip_l_r,
                ),
                dim=-1,
            )
            ...
            pred_pose_, coefficients = self.decoder(
                latent, condition_no_style, slerp_phase
            )
            pred_l_v, pred_l_rot_v = (
                pred_pose_[..., : len(self.pos_rep_idx) * 3],
                pred_pose_[..., len(self.pos_rep_idx) * 3 :],
            )
    ...
    def phase_loss(self, gt_phase, gt_A, gt_F, phase, A, F, sphase):
        loss = {
            "phase": self.mse_loss(gt_phase, phase),
            "A": self.mse_loss(gt_A, A),
            "F": self.mse_loss(gt_F, F),
    ...
    def get_gt_contact(self, gt_v):
        ...
        gt_contact = (
            contact_smooth(gt_fv, idx_smooth, eps, eps_bound)
            + torch.ones_like(gt_fv) * idx
        )
        return gt_contact
    ...
    def shared_forward(self, batch, seq, optimizer=None, d_optimizer=None):
        ...
        glb_loss = self.l1_loss(
            local_pos[:, start_id:target_id, :] / edge_mean,
            pred_pos[:, :, :] / edge_mean,
        )
        glb_rot_loss = self.l1_loss(local_rots[:, start_id:target_id], pred_rot)
        last_pos_loss = self.l1_loss(
            local_pos[:, target_id - 1, :] / edge_mean, pred_pos[:, -1] / edge_mean
        )
        last_rot_loss = self.l1_loss(local_rots[:, target_id - 1, :], pred_rot[:, -1])
        gt_contact = self.get_gt_contact(
            local_pos[:, start_id + 1 : target_id]
            - local_pos[:, start_id : target_id - 1]
        )
        contact_loss = self.contact_foot_loss(
    ...
    def transform_batch_to_filmEncoder(self, glb_pos, glb_rot):
        glb_vel, glb_pos, glb_rot, root_rotation = self.batch_processor.forward(
            glb_rot, glb_pos
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.style_phase = 1.0
        self.test = True
        loss, pred_pos, pred_rot = self.shared_forward(
            batch, self.seq_scheduler.max_seq
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.mode == "pretrain":
            progress = self.common_operator.get_progress(
                self, target_epoch=3, start_epoch=0
            )
            self.schedule_phase = self.common_operator.get_progress(
                self, target_epoch=4, start_epoch=1
            )
            self.style_phase = self.common_operator.get_progress(
                self, target_epoch=6, start_epoch=3
            )
        else:
            progress = 1
            self.schedule_phase = 1.0
    ...
    def configure_optimizers(self):
        ...
        if self.mode == "pretrain" and self.predict_phase == False:
            params = (
                weight_decay("film_generator", lr, 1e-4)
                + weight_decay("target_encoder", lr, 0)
                + weight_decay("embedding_style", lr, 0)
                + weight_decay("offset_encoder", lr, 0)
                + weight_decay("state_encoder", lr, 0)
                + weight_decay("LSTMCell", lr, 0)
                + weight_decay("phase_predictor", lr, 0)
            )  # +weight_decay("decoder",lr,0)
        ...
        if len(non_train_model) > 0:
            ...
            warnings.warn(
                "warning:there are some models not trained:{}".format(non_train_model)
            )
...
class Application_phase(nn.Module):
    ...
    def _get_transform_ori_motion(self, batch):
        ...
        global_rot = self.skeleton.inverse_pos_to_rot(
            global_rot, global_pos, self.offsets, self.tangent
        )
    ...
    def forward(self, t, x):
        self.Net.schedule_phase = 1.0
        self.Net.style_phase = 1.0
        ...
        with torch.no_grad():
            ...
            if self.Net.predict_phase:
                pred_pos, pred_rot, pred_phase, _, _ = self.Net.shift_running(
                    loc_pos,
                    loc_rot,
                    phases,
                    A,
                ...
                pred_pos, pred_rot, pred_phase, _ = self.Net.shift_running(
                    loc_pos,
                    loc_rot,
                    phases,
                    A,
            ...
            output_pos = torch.cat(
                (loc_pos[:, :10, ...], output_pos, loc_pos[:, 10 + seq :, ...]), dim=1
            )
            output_rot = torch.cat(
                (loc_rot[:, :10, ...], output_rot, loc_rot[:, 10 + seq :, ...]), dim=1
            )
            batch = {}
