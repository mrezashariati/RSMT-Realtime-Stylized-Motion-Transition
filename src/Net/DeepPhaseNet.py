...
class PAE_AI4Animation(nn.Module):
    def __init__(self, n_phases, n_joints, length, key_range=1.0, window=2.0):
        ...
        self.freqs = nn.Parameter(
            torch.fft.rfftfreq(time_range)[1:]
...
class Application(nn.Module):
    def __init__(self, Net: DeepPhaseNet, datamodule):
        ...
        self.processor = DeepPhaseProcessor(1 / 30.0)
    ...
    def _transform_anim(self, anim):
        ...
        gv = self.processor.transform_single(
            offsets, hip_pos, quats, self.module.skeleton
        )
        batch = gv
        return batch
